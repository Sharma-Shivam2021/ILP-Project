...
public class DataLoader implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        createAdmin("admin", "AD001", "System Administrator", "9999999999", "admin@hospital.com");
    ...
    private void createAdmin(String username, String code, String name, String phone, String email) {
        User user = userRepository.save(User.builder().username(username)
                .password(passwordEncoder.encode(password)).firstLogin(false).role(Role.ADMIN).build());

        adminRepository.save(Admin.builder().user(user).employeeCode(code)
                .fullName(name).contactPhone(phone).contactEmail(email).build());
    }
    ...
    private String getActualName(String prefix, int index) {
        if ("ICU-N".equals(prefix)) {
            String[] names = { "Aarav Sharma", "Aditi Rao", "Bhavna Patel", "Chirag Gupta", "Deepak Verma" };
            if (index >= 1 && index <= names.length)
                return names[index - 1];
        } else if ("ER-N".equals(prefix)) {
            String[] names = { "Esha Iyer", "Farhan Khan", "Gauri Sen", "Hari Prasad", "Ishaan Malhotra" };
            if (index >= 1 && index <= names.length)
                return names[index - 1];
        } else if ("GW-N".equals(prefix)) {
            String[] names = { "Jaya Reddy", "Karan Johar", "Lata Mangeshkar", "Manoj Bajpayee", "Neha Kakkar" };
            if (index >= 1 && index <= names.length)
                return names[index - 1];
        }
        return "Nurse " + prefix + index;
    }
    ...
    private List<Nurse> generateNurses(String prefix, Department dept, int count) {
        ...
        for (int i = 1; i <= count; i++) {
            ...
            User user = userRepository.save(User.builder().username(username)
                    .password(passwordEncoder.encode(password)).firstLogin(true).role(Role.NURSE).department(dept)
                    .build());

            String actualName = getActualName(prefix, i);
            Nurse nurse = nurseRepository.save(Nurse.builder().user(user).department(dept)
                    .employeeCode(prefix + String.format("%03d", i)).fullName(actualName)
    ...
    private void createIncharge(String username, Department dept, String name, String code) {
        ...
        User user = userRepository.save(User.builder().username(username)
                .password(passwordEncoder.encode(password)).firstLogin(true).role(Role.NURSING_INCHARGE)
    ...
    private void createDutyOfficer(String username, Department dept, String code, String name) {
        ...
        User user = userRepository.save(User.builder().username(username)
                .password(passwordEncoder.encode(password)).firstLogin(true).role(Role.DUTY_OFFICER).department(dept)
                .build());

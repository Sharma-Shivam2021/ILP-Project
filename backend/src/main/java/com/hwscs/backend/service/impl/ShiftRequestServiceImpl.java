...
public class ShiftRequestServiceImpl implements ShiftRequestService {
    ...
    public ShiftRequestResponseDto createRequest(CreateShiftRequestDto dto, String requesterUsername) {
        ...
        // Validation: check if shift change permission is enabled
        if (!Boolean.TRUE.equals(requester.getAllowShiftChange())) {
            throw new InvalidRequestException("Shift swap requests are disabled for you by the Nursing In-charge");
        }
        if (!Boolean.TRUE.equals(peer.getAllowShiftChange())) {
            throw new InvalidRequestException("The selected peer nurse is not allowed to perform shift changes");
        }
        ...
        // Validation: only future or today's shifts can be swapped
        if (requesterShift.getShiftDate().isBefore(LocalDate.now())) {
            throw new InvalidRequestException("Only future or today's shifts can be swapped");
        }
        if (peerShift.getShiftDate().isBefore(LocalDate.now())) {
            throw new InvalidRequestException("Only future or today's shifts can be swapped");
        }
        ...
        ShiftRequest request = ShiftRequest.builder().requesterNurse(requester).peerNurse(peer)
                .requesterNurseShift(requesterShift).peerNurseShift(peerShift).status(RequestStatus.PENDING_PEER)
    ...
    public ShiftRequestResponseDto cancelRequest(Integer requestId, String requesterUsername) {
        ...
        AuditLog audit = AuditLog.builder()
                .action("SHIFT_CANCELLATION")
                .performedBy(requesterUsername)
                .targetUser(request.getPeerNurse().getUser().getUsername())
                .details("Cancelled shift swap request ID: " + request.getId() + " between " + requester.getUser().getUsername() + " and " + request.getPeerNurse().getUser().getUsername())
                .build();
    ...
    public ShiftRequestResponseDto peerResponse(PeerResponseDto dto, String peerUsername) {
        ...
        // Validation: check if shift change permission is enabled
        if (!Boolean.TRUE.equals(peer.getAllowShiftChange())) {
            throw new InvalidRequestException("Shift changes are disabled for you by the Nursing In-charge");
        }
        if (!Boolean.TRUE.equals(request.getRequesterNurse().getAllowShiftChange())) {
            throw new InvalidRequestException("The requester nurse is no longer allowed to perform shift changes");
        }

        // Can only respond when request is in PENDING_PEER state
        if (request.getStatus() != RequestStatus.PENDING_PEER) {
            throw new InvalidRequestException(
                    "This request is not pending peer response. Current status: " + request.getStatus());
        }
    ...
    public ShiftRequestResponseDto inchargeReview(InchargeApprovalDto dto, String inchargeUsername) {
        ...
        if (dto.isApproved()) {
            ...
            // Execute the shift swap: swap the Shift references between the two NurseShift
            // records
            executeShiftSwap(request);

            // Audit Log
            AuditLog audit = AuditLog.builder()
                    .action("SHIFT_APPROVAL")
                    .performedBy(inchargeUsername)
                    .targetUser(request.getRequesterNurse().getUser().getUsername())
                    .details("Approved shift swap request ID: " + request.getId() + " between " + request.getRequesterNurse().getUser().getUsername() + " and " + request.getPeerNurse().getUser().getUsername())
                    .build();
            auditLogRepository.save(audit);
            ...
            // Audit Log
            AuditLog audit = AuditLog.builder()
                    .action("SHIFT_REJECTION")
                    .performedBy(inchargeUsername)
                    .targetUser(request.getRequesterNurse().getUser().getUsername())
                    .details("Rejected shift swap request ID: " + request.getId() + " between " + request.getRequesterNurse().getUser().getUsername() + " and " + request.getPeerNurse().getUser().getUsername())
                    .build();
            auditLogRepository.save(audit);
        }
    ...
    public List<ShiftRequestResponseDto> getPendingInchargeReview(String inchargeUsername) {
        ...
        return shiftRequestRepository.findByDepartmentAndStatus(incharge.getDepartment(), RequestStatus.PEER_ACCEPTED)
    ...
    public List<EligiblePeerDto> getEligiblePeers(Integer requesterShiftId, String requesterUsername) {
        ...
        // Future or today's shift validation
        if (requesterShift.getShiftDate().isBefore(LocalDate.now())) {
            throw new InvalidRequestException("Only future or today's shifts are eligible for swaps");
        }

        List<NurseShift> eligibleShifts = nurseShiftRepository.findEligiblePeers(requester.getDepartment(),
                requesterShift.getShiftDate(), requester.getId(), requesterShift.getShift().getId());

        return eligibleShifts.stream()
                .filter(ns -> Boolean.TRUE.equals(ns.getNurse().getAllowShiftChange()))
                .map(ns -> EligiblePeerDto.builder()
                        ...
                        .employeeCode(ns.getNurse().getEmployeeCode())
                        .contactEmail(ns.getNurse().getContactEmail())
                        .contactPhone(ns.getNurse().getContactPhone())
                        .departmentName(ns.getNurse().getDepartment().getName())
                        .nurseType(ns.getNurse().getNurseType().name())
                        .build())
                .toList();
    }
    ...
    public List<RequestHistoryResponseDto> getRequestHistory(Integer requestId, String username) {
        ...
        User currentUser = request.getRequesterNurse().getUser().getUsername().equals(username)
        ...
        if (currentUser == null) {
            ...
            if (incharge != null
                    && incharge.getDepartment().getId().equals(request.getRequesterNurse().getDepartment().getId())) {
        ...
        if (currentUser == null) {
            ...
            if (dutyOfficer != null && dutyOfficer.getDepartment().getId()
        ...
        return requestHistoryRepository.findByShiftRequest(request).stream()
                .map(h -> RequestHistoryResponseDto.builder().id(h.getId())
                        .actorUsername(h.getActorUser().getUsername()).actorRole(h.getActorUser().getRole().name())
                        .action(h.getAction().name()).remarks(h.getRemarks()).actedAt(h.getActedAt()).build())
                .toList();
    }
    ...
    private void saveHistory(ShiftRequest request, User actor, RequestAction action, String remarks) {
        RequestHistory history = RequestHistory.builder().shiftRequest(request).actorUser(actor).action(action)
    ...
    public ShiftRequestResponseDto mapToDto(ShiftRequest r) {
        ...
        if (r.getStatus() == RequestStatus.PEER_REJECTED || r.getStatus() == RequestStatus.REJECTED || r.getStatus() == RequestStatus.CANCELLED) {
            List<RequestHistory> histories = requestHistoryRepository.findByShiftRequest(r);
            if (histories != null && !histories.isEmpty()) {
                rejectionReason = histories.stream()
                        .filter(h -> h.getAction() == RequestAction.PEER_REJECTED
                                || h.getAction() == RequestAction.REJECTED
                                || h.getAction() == RequestAction.CANCELLED)
                        .max((h1, h2) -> {
                            if (h1.getActedAt() == null && h2.getActedAt() == null) return 0;
                            if (h1.getActedAt() == null) return -1;
                            if (h2.getActedAt() == null) return 1;
                            return h1.getActedAt().compareTo(h2.getActedAt());
                        })
                        .map(RequestHistory::getRemarks)
                        .orElse(null);
            }
        }

        return ShiftRequestResponseDto.builder().id(r.getId()).status(r.getStatus().name()).remarks(r.getRemarks())
                .rejectionReason(rejectionReason)
                .createdAt(r.getCreatedAt())
                // Requester
                .requesterNurseId(r.getRequesterNurse().getId()).requesterNurseName(r.getRequesterNurse().getFullName())
                .requesterShiftDate(rs.getShiftDate()).requesterShiftName(rs.getShift().getShiftName())
                .requesterShiftStart(rs.getShift().getStartTime()).requesterShiftEnd(rs.getShift().getEndTime())
                // Peer
                .peerNurseId(r.getPeerNurse().getId()).peerNurseName(r.getPeerNurse().getFullName())
                .peerShiftDate(ps.getShiftDate()).peerShiftName(ps.getShift().getShiftName())
                .peerShiftStart(ps.getShift().getStartTime()).peerShiftEnd(ps.getShift().getEndTime()).build();
    }
}